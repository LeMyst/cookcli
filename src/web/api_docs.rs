...
"#,
            ...
            ep(
                ...
                "Parses the recipe and returns its ingredients, cookware, timers and steps. \
                 ...
                 level of `recipe`. The `image` field is a URL under `/api/static/`, after the \
                 server's `--url-prefix` if one is set, when the recipe has a title image, \
                 otherwise null. Frontmatter lands under \
